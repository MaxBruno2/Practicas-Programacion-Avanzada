...
class ThreadSocket(QThread):
    ...
    reproducir_read = Signal(int)
    adelante_read = Signal(int)
    boton_read = Signal(int)
    detener_read = Signal(int)
                ...
                if data:
                    for message in data.splitlines():  # separa cada línea
                        try:
                            ...
                            elif message.startswith('<reproducir>'):
                                reproducir = int(message.removeprefix('<reproducir>'))
                                self.reproducir_read.emit(reproducir)
                            ...
                            elif message.startswith('<detener>'):
                                detener = int(message.removeprefix('<detener>'))
                                self.detener_read.emit(detener)
                        except ValueError:
                            pass
        except ...:
            pass
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.ejeX_actual = 2047
        self.ejeY_actual = 2047
        ...
        self.hilo.reproducir_read.connect(self.reproducir_read)
        ...
        self.hilo.detener_read.connect(self.salir)
    ...
    def reproducir_read(self, value):
